...
add_action('save_post', 'relevanssi_publish', 99);				// thanks to Brian D Gajus
...
function relevanssi_do_query(&$query) {
	...
	if (isset($query->query_vars['searchblogs'])) {
		...
		if ($cache) {
			$params = md5(serialize(array($q, $cat, $excat, $tag, $expids, $post_type, $tax, $tax_term, $operator, $search_blogs, $customfield_key, $customfield_value)));
			$return = relevanssi_fetch_hits($params);
			if (!$return) {
				$return = relevanssi_search($q, $cat, $excat, $tag, $expids, $post_type, $tax, $tax_term, $operator, $search_blogs, $customfield_key, $customfield_value);
...
function relevanssi_search($q, $cat = NULL, $excat = NULL, $tag = NULL, $expost = NULL, $post_type = NULL, $taxonomy = NULL, $taxonomy_term = NULL, $operator = "AND", $search_blogs = NULL, $customfield_key = NULL, $customfield_value = NULL) {
...
function relevanssi_index_doc($indexpost, $remove_first = false, $custom_fields = false) {
	...
	$hide = get_post_meta($post->ID, 'relevanssi_hide_post', true);
	if ("on" == $hide) return;
	...
	if ("on" == get_option("relevanssi_include_cats")) {
		...
		if (is_array($post_categories)) {
			foreach ($post_categories as $p_cat) {
				...
				$cat_tokens = relevanssi_tokenize($cat_name, true, $min_word_length);
	...
	if (apply_filters('relevanssi_index_content', $index_content)) {
		...
		$titles = relevanssi_tokenize($post->post_title, true, $min_word_length);
	...
	foreach ($insert_data as $term => $data) {
		...
		$value = $wpdb->prepare("(%d, %s, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %s)",
	...
	if (!empty($values)) {
		...
		$query = "INSERT IGNORE INTO $relevanssi_table (doc, term, content, title, comment, tag, link, author, category, excerpt, taxonomy, customfield, type)
			VALUES $values";
		$wpdb->query($query);
	}
...
function relevanssi_options_form() {
	...
	$serialize_options['relevanssi_tag_boost'] = $tag_boost;
...
<div class='postbox-container' style='width:70%;'>
	...
	<small><?php _e("If you want date-based results, you really should do what <a href='http://www.relevanssi.com/knowledge-base/relevanssi-match/'>this knowledge base entry</a> says instead of changing this option.", "relevanssi"); ?></small>
	...
	<tr>
		<td>
			<?php _e('Tags', 'relevanssi'); ?> 
		</td>
		<td>
			<input type='text' name='relevanssi_tag_boost' size='4' value='<?php echo $tag_boost ?>' />
		</td>
		<td>
			<?php echo $tag_boost_default; ?>
		</td>
	</tr>
	<tr>
		<td>
...
function relevanssi_save_postdata($post_id) {
	...
	update_post_meta($post_id, 'relevanssi_hide_post', $hide);
}
