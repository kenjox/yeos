    ...
    if (!is_front_page()) {
        ...
        if (is_single()) { //check if any single post is being displayed.
            ...
            //then the post is listed in more than 1 category.
                ...
                echo "<li>"; the_category( $delimiter1, multiple)."</li>";
                    ...
                    if (strlen(get_the_title()) >= $maxLength) { //If the title is long, then don't display it all.
                        echo '<li class="current">' . $delimiter . trim(substr(get_the_title(), 0, $maxLength)) . ' ...';
                        echo '<li class="current"><a href="' . get_permalink() . '">' . trim(substr(get_the_title(), 0, $maxLength)) . ' ...</a></li>';
                    }
        ...
        elseif ( is_page() && $post->post_parent ) {  //Check if this is a subpage (submenu) being displayed.
            ...
            foreach($post_array as $key=>$postid){
                ...
                echo '<li><a href="' . get_permalink($post_ids) . '">' . $title . '</a></li>' . $delimiter;
            }
            echo '<li class="current"><a href="' . get_permalink() . '">' . get_the_title() . '</a></li>'; //returns the title of the current page.
        }
